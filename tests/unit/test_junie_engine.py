"""
Tests for the JetBrains Junie CLI coding engine connector.
...
ENGINE_NAME = "junie"
DEFAULT_MODEL = ""
DEFAULT_BINARY = "/usr/local/bin/junie"


def _make_engine(**overrides) -> JunieCLIEngine:
    """Factory to create a JunieCLIEngine with sensible defaults."""
    defaults = {
    ...
    return JunieCLIEngine(**defaults)
...
class TestEngineIdentity:
    ...
    def test_name_is_junie(self):
        engine = _make_engine()
        assert engine.name == "junie"
...
# ===========================================================================
# Command building -- Junie uses positional prompt arg
# ===========================================================================
...
class TestCommandBuilding:
    ...
    def test_binary_is_first_arg(self, mock_run):
        ...
        engine = _make_engine(binary="/usr/local/bin/junie")
        engine.run("prompt", model=DEFAULT_MODEL, timeout=60)

        cmd = mock_run.call_args[0][0]
        assert cmd[0] == "/usr/local/bin/junie"

    @patch(f"{ENGINE_MODULE}.subprocess.run")
    def test_prompt_as_positional_arg(self, mock_run):
        """Junie takes the prompt as a positional argument, not --message."""
        mock_run.return_value = MagicMock(
            stdout="ok", stderr="", returncode=0,
        )
        engine = _make_engine()
