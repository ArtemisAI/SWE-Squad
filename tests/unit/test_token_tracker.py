...
class TestAdaptiveTimeout:
    def test_returns_default_when_no_samples(self):
        at = AdaptiveTimeout(300, min_samples=5)
        assert at.get() == 300
    ...
    def test_returns_adaptive_value_with_enough_samples(self):
        at = AdaptiveTimeout(300, min_val=60, max_val=3600, min_samples=5)
        for _ in range(5):
            at.record(100.0)
        assert at.get() == 100

    def test_adaptive_value_clamped_to_min(self):
        at = AdaptiveTimeout(300, min_val=200, max_val=3600, min_samples=5)
        for _ in range(5):
            at.record(10.0)
        assert at.get() == 200

    def test_adaptive_value_clamped_to_max(self):
        at = AdaptiveTimeout(300, min_val=60, max_val=500, min_samples=5)
        for _ in range(5):
            at.record(1000.0)
        assert at.get() == 500

    def test_sliding_window_evicts_old_samples(self):
        at = AdaptiveTimeout(300, min_val=60, max_val=3600, window=5, min_samples=3)
        for _ in range(5):
            at.record(100.0)
        for _ in range(5):
            at.record(200.0)
    ...
    def test_mean_plus_stdev_formula(self):
        import math
        at = AdaptiveTimeout(300, min_val=60, max_val=3600, min_samples=5)
        samples = [100.0, 150.0, 200.0, 120.0, 180.0]
        for s in samples:
            at.record(s)
        mean = sum(samples) / len(samples)
        variance = sum((x - mean) ** 2 for x in samples) / len(samples)
        stdev = math.sqrt(variance)
        expected = int(mean + 1.5 * stdev)
        assert at.get() == expected

    def test_sample_count(self):
        at = AdaptiveTimeout(300)
    ...
    def test_thread_safety(self):
        import threading
        at = AdaptiveTimeout(300, min_val=60, max_val=3600, window=20, min_samples=5)
        threads = [threading.Thread(target=lambda: at.record(float(i))) for i in range(20)]
        for t in threads:
            t.start()
        for t in threads:
            t.join()
        assert at.sample_count == 20
