"""
Tests for the Antigravity CLI coding engine connector.
...
defaults) plus Antigravity-specific tests for model flag and command building.
"""
...
DEFAULT_BINARY = "/usr/local/bin/antigravity"


def _make_engine(**overrides) -> AntigravityCLIEngine:
    """Factory to create an AntigravityCLIEngine with sensible defaults."""
    defaults = {
    ...
    return AntigravityCLIEngine(**defaults)
...
class TestEngineIdentity:
    ...
    def test_name_is_antigravity(self):
        engine = _make_engine()
...
class TestEngineRunSuccess:
    ...
    def test_run_uses_model_override(self, mock_run):
        ...
        engine = _make_engine()
        result = engine.run("prompt", model="gemini-2.0-flash", timeout=60)
...
class TestEngineDefaults:
    ...
    def test_custom_default_model(self):
        engine = _make_engine(default_model="gemini-2.0-flash")
    ...
    def test_default_model_used_when_no_override(self, mock_run):
        ...
        engine = _make_engine(default_model="gemini-2.5-pro")
...
# ===========================================================================
# Command building -- Antigravity-specific
# ===========================================================================


class TestAntigravityCommandBuilding:
    """Verify the subprocess command includes Antigravity-specific flags."""
    ...
    def test_binary_is_first_arg(self, mock_run):
        ...
        engine = _make_engine(binary="/usr/local/bin/antigravity")
        engine.run("prompt", model=DEFAULT_MODEL, timeout=60)

        cmd = mock_run.call_args[0][0]
        assert cmd[0] == "/usr/local/bin/antigravity"
    ...
    def test_model_flag_in_command(self, mock_run):
        ...
        engine = _make_engine()
        engine.run("prompt", model="gemini-2.0-flash", timeout=60)
