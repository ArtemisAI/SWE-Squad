"""
Tests for the Kilo Code CLI coding engine connector.
...
ENGINE_NAME = "kilo"
DEFAULT_MODEL = ""
DEFAULT_BINARY = "/usr/local/bin/kilo"


def _make_engine(**overrides) -> KiloCLIEngine:
    """Factory to create a KiloCLIEngine with sensible defaults."""
    defaults = {
...
class TestCommandBuilding:
    ...
    def test_binary_is_first_arg(self, mock_run):
        ...
        engine = _make_engine(binary="/usr/local/bin/kilo")
        engine.run("prompt", model=DEFAULT_MODEL, timeout=60)

        cmd = mock_run.call_args[0][0]
        assert cmd[0] == "/usr/local/bin/kilo"
