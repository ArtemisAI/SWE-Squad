"""
Tests for the Cline CLI coding engine connector.
...
ENGINE_MODULE = "src.swe_team.providers.coding_engine.cline"

ENGINE_NAME = "cline"
DEFAULT_MODEL = ""
DEFAULT_BINARY = "/usr/local/bin/cline"


def _make_engine(**overrides) -> ClineCLIEngine:
    """Factory to create a ClineCLIEngine with sensible defaults."""
    defaults = {
    ...
    return ClineCLIEngine(**defaults)
...
class TestEngineIdentity:
    ...
    def test_name_is_cline(self):
        engine = _make_engine()
        assert engine.name == "cline"
...
class TestCommandBuilding:
    ...
    def test_binary_is_first_arg(self, mock_run):
        ...
        engine = _make_engine(binary="/usr/local/bin/cline")
        engine.run("prompt", model=DEFAULT_MODEL, timeout=60)
    ...
    @patch(f"{ENGINE_MODULE}.subprocess.run")
    def test_task_and_auto_approve_flags_in_command(self, mock_run):
        mock_run.return_value = MagicMock(
            stdout="ok", stderr="", returncode=0,
        )
        engine = _make_engine()
    ...
    @patch(f"{ENGINE_MODULE}.subprocess.run")
    def test_cwd_flag_in_command(self, mock_run):
        mock_run.return_value = MagicMock(
            stdout="ok", stderr="", returncode=0,
        )
        engine = _make_engine()
    ...
    @patch(f"{ENGINE_MODULE}.subprocess.run")
    def test_session_flag_in_command(self, mock_run):
        mock_run.return_value = MagicMock(
            stdout="ok", stderr="", returncode=0,
        )
        engine = _make_engine()
        engine.run("prompt", model=DEFAULT_MODEL, timeout=60, session_id="sess-1")
        cmd = mock_run.call_args[0][0]
        assert "--session" in cmd
        assert "sess-1" in cmd

    @patch(f"{ENGINE_MODULE}.subprocess.run")
    def test_no_edit_flag_when_enabled(self, mock_run):
        mock_run.return_value = MagicMock(
            stdout="ok", stderr="", returncode=0,
        )
        engine = _make_engine(no_edit=True)
        engine.run("prompt", model=DEFAULT_MODEL, timeout=60)
        cmd = mock_run.call_args[0][0]
...
class TestOutputParsing:
    """Validate Cline JSON output parsing for audit and cost tracking."""

    @patch(f"{ENGINE_MODULE}.subprocess.run")
    def test_extracts_usage_and_session(self, mock_run):
        mock_run.return_value = MagicMock(
            stdout=(
                '{"summary":"done","session_id":"abc123","usage":{"input_tokens":12,'
                '"output_tokens":34,"cost_usd":0.56}}'
            ),
            stderr="",
            returncode=0,
        )
        ...
        assert result.cost_usd == 0.56

    @patch(f"{ENGINE_MODULE}.subprocess.run")
    def test_extracts_tool_use_log_to_metadata(self, mock_run):
        mock_run.return_value = MagicMock(
            stdout='{"summary":"done","tool_use":[{"tool":"edit","ok":true}]}',
            stderr="",
            returncode=0,
        )
        engine = _make_engine()
        result = engine.run("prompt", model=DEFAULT_MODEL, timeout=60)
        assert "tool_use" in result.metadata
        assert isinstance(result.metadata["tool_use"], list)

    @patch(f"{ENGINE_MODULE}.subprocess.run")
    def test_marks_summary_fallback_when_no_summary_key(self, mock_run):
        mock_run.return_value = MagicMock(
            stdout='{"unstructured":"raw"}',
            stderr="",
            returncode=0,
        )
        engine = _make_engine()
        result = engine.run("prompt", model=DEFAULT_MODEL, timeout=60)
        assert result.metadata.get("summary_fallback") == "raw_stdout"
...
class TestRegistryIntegration:
    ...
    def test_cline_in_registry(self):
        from src.swe_team.providers.coding_engine import list_engines
        engines = list_engines()
        assert "cline" in engines

    def test_cline_resolve(self):
        from src.swe_team.providers.coding_engine import resolve_engine
        engine = resolve_engine("cline", {"timeout_seconds": 60})
        assert engine.name == "cline"

    @patch(f"{ENGINE_MODULE}.subprocess.run")
    def test_cline_resolve_honors_headless_flags(self, mock_run):
        mock_run.return_value = MagicMock(
            stdout="ok", stderr="", returncode=0,
        )
        from src.swe_team.providers.coding_engine import resolve_engine
        engine = resolve_engine(
            "cline",
            {"timeout_seconds": 60, "auto_approve": False, "no_edit": True},
        )
        engine.run("prompt", model=DEFAULT_MODEL, timeout=60)
        cmd = mock_run.call_args[0][0]
        assert "--no-edit" in cmd
