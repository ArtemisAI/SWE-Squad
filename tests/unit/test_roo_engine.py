"""
Tests for the Roo Code CLI coding engine connector.
...
ENGINE_NAME = "roo"
DEFAULT_MODEL = ""
DEFAULT_BINARY = "/usr/local/bin/roo"


def _make_engine(**overrides) -> RooCLIEngine:
    """Factory to create a RooCLIEngine with sensible defaults."""
    defaults = {
...
class TestEngineIdentity:
    ...
    def test_name_is_lowercase(self):
        ...
        assert engine.name == engine.name.lower(), (
            ...
            "Use lowercase identifiers (e.g., 'roo', not 'Roo')."
        )

    def test_name_is_roo(self):
        engine = _make_engine()
        assert engine.name == "roo"
...
class TestCommandBuilding:
    ...
    def test_binary_is_first_arg(self, mock_run):
        ...
        engine = _make_engine(binary="/usr/local/bin/roo")
        engine.run("prompt", model=DEFAULT_MODEL, timeout=60)

        cmd = mock_run.call_args[0][0]
        assert cmd[0] == "/usr/local/bin/roo"
