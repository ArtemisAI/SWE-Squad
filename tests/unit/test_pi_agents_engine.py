"""
Tests for the Pi Agents CLI coding engine connector.
...
defaults) plus Pi-specific tests for command building.
"""
...
ENGINE_NAME = "pi_agents"
DEFAULT_MODEL = ""
DEFAULT_BINARY = "/usr/local/bin/pi"


def _make_engine(**overrides) -> PiAgentsCLIEngine:
    """Factory to create a PiAgentsCLIEngine with sensible defaults."""
    defaults = {
    ...
    return PiAgentsCLIEngine(**defaults)
...
class TestEngineIdentity:
    ...
    def test_name_is_pi_agents(self):
        engine = _make_engine()
        assert engine.name == "pi_agents"
...
class TestEngineRunSuccess:
    ...
    def test_run_success_returns_engine_result(self, mock_run):
        mock_run.return_value = MagicMock(
            stdout="task completed", stderr="", returncode=0,
        )
        engine = _make_engine()
...
class TestEngineDefaults:
    ...
    def test_custom_default_model(self):
        engine = _make_engine(default_model="pi-model")
        assert engine.model() == "pi-model"
...
# ===========================================================================
# Command building -- Pi-specific
# ===========================================================================


class TestPiAgentsCommandBuilding:
    """Verify the subprocess command includes Pi-specific structure."""
    ...
    def test_binary_is_first_arg(self, mock_run):
        ...
        engine = _make_engine(binary="/usr/local/bin/pi")
        engine.run("prompt", model=DEFAULT_MODEL, timeout=60)
    ...
    @patch(f"{ENGINE_MODULE}.subprocess.run")
    def test_prompt_flag_present(self, mock_run):
        mock_run.return_value = MagicMock(
            stdout="ok", stderr="", returncode=0,
        )
        engine = _make_engine()
    ...
    def test_no_model_flag_when_model_empty(self, mock_run):
        """Pi does not support model selection -- no --model flag."""
        mock_run.return_value = MagicMock(
            stdout="ok", stderr="", returncode=0,
        )
        engine = _make_engine()
...
class TestRegistryIntegration:
    ...
    def test_pi_agents_in_registry(self):
        from src.swe_team.providers.coding_engine import list_engines
        engines = list_engines()
        assert "pi_agents" in engines

    def test_pi_agents_resolve(self):
        from src.swe_team.providers.coding_engine import resolve_engine
        engine = resolve_engine("pi_agents", {"timeout_seconds": 60})
        assert engine.name == "pi_agents"
