"""
Tests for the Devin coding engine connector.
...
Devin-specific tests for API request building and error handling.
"""
...
ENGINE_MODULE = "src.swe_team.providers.coding_engine.devin"
...
def _make_engine(**overrides) -> DevinEngine:
    """Factory to create a DevinEngine with sensible defaults."""
    defaults = {
...
class TestEngineIdentity:
    ...
    def test_name_is_devin(self):
        engine = _make_engine()
        assert engine.name == "devin"
...
class TestEngineAvailability:
    ...
    def test_is_available_true_with_api_url(self):
        engine = _make_engine(api_url="https://api.devin.ai")
...
class TestEngineDefaults:
    ...
    def test_custom_default_model(self):
        engine = _make_engine(default_model="devin-v2")
        assert engine.model() == "devin-v2"
...
# ===========================================================================
# API request building -- Devin-specific
# ===========================================================================


class TestDevinRequestBuilding:
    ...
    def test_request_url(self, mock_urlopen):
        ...
        req = mock_urlopen.call_args[0][0]
        assert req.full_url == "https://api.devin.ai/v1/sessions"
    ...
    def test_request_body_includes_model_when_set(self, mock_urlopen):
        mock_urlopen.return_value = _mock_response({"result": "ok"})
        engine = _make_engine()
        engine.run("prompt", model="devin-v2", timeout=60)

        req = mock_urlopen.call_args[0][0]
        body = json.loads(req.data.decode("utf-8"))
        assert body["model"] == "devin-v2"
