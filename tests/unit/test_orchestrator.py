...
class TestModelBoundary:
    ...
    def test_code_task_with_claude_model_allowed(self):
        mock_engine = MagicMock()
        mock_engine.run.return_value = MagicMock(stdout="result", returncode=0, success=True)
        self.agent._engine = mock_engine
        ticket = _make_ticket()
    ...
    def test_non_code_task_skips_boundary_check(self):
        """Tasks without code-generation keywords skip boundary enforcement."""
        mock_engine = MagicMock()
        mock_engine.run.return_value = MagicMock(stdout="analysis", returncode=0, success=True)
        self.agent._engine = mock_engine
        ticket = _make_ticket()
...
class TestUpdateProgress:
    ...
    @patch("src.swe_team.github_integration.update_github_comment")
    ...
    @patch("src.swe_team.github_integration.update_github_comment", side_effect=Exception("API error"))
