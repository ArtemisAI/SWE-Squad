"""
Tests for the Jules CLI coding engine connector.
...
defaults) plus Jules-specific tests for command building.
"""
...
ENGINE_NAME = "jules"
DEFAULT_MODEL = ""
DEFAULT_BINARY = "/usr/local/bin/jules"


def _make_engine(**overrides) -> JulesCLIEngine:
    """Factory to create a JulesCLIEngine with sensible defaults."""
    defaults = {
    ...
    return JulesCLIEngine(**defaults)
...
class TestEngineIdentity:
    ...
    def test_name_is_jules(self):
        engine = _make_engine()
        assert engine.name == "jules"
...
# ===========================================================================
# Command building -- Jules-specific
# ===========================================================================


class TestJulesCommandBuilding:
    """Verify the subprocess command includes Jules-specific structure."""
    ...
    def test_binary_is_first_arg(self, mock_run):
        ...
        engine = _make_engine(binary="/usr/local/bin/jules")
        engine.run("prompt", model=DEFAULT_MODEL, timeout=60)

        cmd = mock_run.call_args[0][0]
        assert cmd[0] == "/usr/local/bin/jules"

    @patch(f"{ENGINE_MODULE}.subprocess.run")
    def test_prompt_is_direct_argument(self, mock_run):
        mock_run.return_value = MagicMock(
            stdout="ok", stderr="", returncode=0,
        )
        engine = _make_engine()
    ...
    def test_no_model_flag_when_model_empty(self, mock_run):
        """Jules does not support model selection -- no --model flag."""
        mock_run.return_value = MagicMock(
            stdout="ok", stderr="", returncode=0,
        )
        engine = _make_engine()
