...
import os
store = SupabaseTicketStore(supabase_url=os.environ['SUPABASE_URL'], supabase_key=os.environ['SUPABASE_ANON_KEY'], team_id=os.environ['SWE_TEAM_ID'])
...
**Root cause:** `comment_on_github_issue()` was missing a `repo` parameter, so `gh` defaulted to the CWD repo (SWE-Squad) instead of the ticket's target repo.
...
**Symptom:** `WARNING: rsync from worker-* failed` or `Timeout collecting logs from` in `swe_team.log`. Monitor cycle runs but only scans local logs.

**Diagnosis:**
```bash
# Check SSH connectivity to all workers (use aliases from config/ssh_workers.conf)
ssh -F config/ssh_workers.conf worker-1 "echo ok"
ssh -F config/ssh_workers.conf worker-2 "echo ok"

# Check that log directory exists on the worker
ssh -F config/ssh_workers.conf worker-1 "ls -la ~/Projects/YourApp/logs/"
...
**Common causes:**
- Worker node is offline or VPN is disconnected → check network connectivity
- SSH key not in worker's `authorized_keys` → `ssh-copy-id -i ~/.ssh/swe_workers_key.pub agent@<IP>`
- Log directory doesn't exist on worker → exclude from `swe_team.yaml`
...
bash scripts/ops/propagate.sh

# Check propagation log
tail -20 logs/propagate.log

# Verify worker is on the correct commit
ssh -F config/ssh_workers.conf worker-1 "cd ~/Projects/YourApp && git log --oneline -3"
```
...
### 2.13 Developer keep/discard loop discards all fixes (0% success)

**Symptom:** `swe_team.log` shows `Discarding fix — no changes detected` for every development attempt, even when Claude CLI actually committed a valid fix.

**Root cause:** The keep/discard loop compared the working tree diff instead of comparing `HEAD` to `base_sha`. Claude CLI commits changes directly, so the working tree appears clean and the loop concludes nothing changed.

**Fix (applied 2026-03-28, PR #333):** `developer.py` now records `base_sha = git rev-parse HEAD` before invoking Claude CLI, then checks `git rev-parse HEAD != base_sha` to detect commits made by the CLI.

**Prevention:** When checking for agent-produced changes, always compare commit SHAs rather than relying on working tree status. Claude CLI (and other coding engines) may commit directly.

---

### 2.14 Cross-repo contamination in GitHub issue creation

**Symptom:** Issues meant for a sandbox repo (e.g., `HealthTrack`) are created in the SWE-Squad repo instead, or comments appear on the wrong repo's issues.

**Root cause:** `create_github_issue()` fell back to the `SWE_GITHUB_REPO` environment variable when the ticket's repo field was empty or missing. Monitor-detected tickets often lacked explicit repo metadata.

**Fix (applied 2026-03-28, PR #333):** `create_github_issue()` no longer falls back to `SWE_GITHUB_REPO` for monitor-detected tickets. If no repo is available, the function raises rather than silently routing to the wrong repo.

**Prevention:** Always pass `repo=ticket.metadata.get("repo", "")` explicitly. Verify the repo value is non-empty before calling any GitHub integration function.

---

### 2.15 Development runs in SWE-Squad worktree instead of sandbox repo

**Symptom:** Developer agent creates branches and commits inside the SWE-Squad repo instead of the actual target sandbox repo. Fixes never reach the correct codebase.

**Root cause:** Prior to sandbox routing, the developer agent defaulted to the SWE-Squad worktree for all development work regardless of ticket origin.

**Fix (applied 2026-03-28, PR #333):** `RepoRouter` maps each ticket to its configured sandbox repo path. The developer agent sets `cwd` to the sandbox repo clone before invoking Claude CLI. Unknown repos raise `ValueError` (fail-closed).
...
print(router.list_repos())
"
```

**Prevention:** All new sandbox repos must be added to `config/swe_team.yaml` under `repos:` with a valid local `clone_path`. The `RepoRouter` will reject any ticket targeting an unconfigured repo.
...
git -C /path/to/sandbox-repo reset --merge 2>/dev/null || true
